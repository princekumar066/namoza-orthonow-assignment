...
A critical issue in this setup is contact deduplication. HubSpot deduplicates contacts by email address by default, but OrthoNow's form collects only name and
phone with no email field. Without custom handling, every form submission creates a new contact record regardless of whether that patient already exists,
